...
static DNCameraManager *_cameraManager;

@interface DNCameraManager ()

@property (nonatomic, strong, readwrite) GPUImageStillCamera *camera;
@property (nonatomic, weak) GPUImageView *outputView;
@property (nonatomic, strong, readwrite) DNFilterHandler *currentFilterHandler;
...
@implementation DNCameraManager

+ (DNCameraManager *)shareManager {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _cameraManager = [[DNCameraManager alloc] init];
    });
...
- (void)setupMovieWriter {
    NSString *videoPath = [DNFileHelper randomFilePathInTmpWithSuffix:@".m4v"];
...
- (void)setupFilterHandler {
    self.currentFilterHandler = [[DNFilterHandler alloc] init];
    // 添加效果滤镜
